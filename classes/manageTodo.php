<?php
	...
	class ManageTodo{
		...
		function createTodo($username,$title,$description,$due_date,$create_date,$label){
			$query=$this->link->prepare("INSERT INTO todo (username,title,description,due_date,create_date,label)VALUES(?,?,?,?,?,?)");
		...
		function countTodo($username,$label){
			$query=$this->link0>quert("SELECT count(*) AS TOTAL_TODO FROM todo WHERE username='$username' AND label='$label' ");
